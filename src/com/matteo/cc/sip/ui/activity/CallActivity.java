...
public class CallActivity extends Activity implements FunctionsListener,
	...
	private void makeCall(){
		...
		if(result==Constant.Status.OK){
			...
			Toast.makeText(this, this.getResources().getString(R.string.call_out_failed), Toast.LENGTH_SHORT).show();
			this.closeActivity();
		}
	}
	...
	private void updateCallInfo(XCallInfo newCallInfo){
		...
		if(newCallInfo.getState()==this.mCallInfo.getState()&&newCallInfo.getDirect()==this.mCallInfo.getDirect()){
			return;
		}
	...
	public static void startActivity(Context context, String number) {
		CallActivity.startActivity(context,  new XCallInfo(number, Constant.CallDirect.OUT, Constant.CallState.OUT_NOT_START));
	}
	...
	class CallStateReceiver extends BroadcastReceiver{
		...
		public void onReceive(Context context, Intent intent) {
			XCallInfo newCallInfo=intent.getParcelableExtra(Constant.Key.CALL_INFO);
			if(newCallInfo==null){
				return;
			}
